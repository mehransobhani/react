                ...
                productsInformation.map((product, key) => {
                    return (
                        <div key={key} className={['row', 'p-2', 'mt-2', 'align-items-center', 'text-right', 'rtl'].join(' ')} style={{background: '#F2F2F2', borderRadius: '2px'}}>
                            <h6 className={['col-4', 'text-rigt', 'mb-0', 'font14'].join(' ')}>{product.productName}</h6>
                            <h6 className={['col-1', 'text-center', 'mb-0', 'font14'].join(' ')}>{product.productStock}</h6>
                            <h6 className={['col-1', 'text-center', 'mb-0', 'font14'].join(' ')}>{product.productBuyPrice.toLocaleString()}</h6>
                            <h6 className={['col-1', 'text-center', 'mb-0', 'font14'].join(' ')}>{product.productPrice.toLocaleString()}</h6>
                            ...
                            <input type="number" defaultValue='' onFocus={()=>{focusOnProduct(key)}} onChange={productFinalStockInputChanged} placeholder='موجودی' className={['col-1', 'text-center', 'font14'].join(' ')} style={{borderStyle: 'none', outlineStyle: 'none', borderRadius: '2px'}}/>
                            <input type="number" defaultValue='' onFocus={()=>{focusOnProduct(key)}} onChange={productPercentInputChanged} placeholder='درصد' className={['col-1', 'text-center', 'font14'].join(' ')} style={{borderStyle: 'none', outlineStyle: 'none', borderRadius: '2px'}}/>
                            <input type="number" defaultValue='' onFocus={()=>{focusOnProduct(key)}} onChange={productPriceInputChanged} placeholder='قیمت' className={['col-1', 'text-center', 'font14'].join(' ')} style={{borderStyle: 'none', outlineStyle: 'none', borderRadius: '2px'}}/>
                        </div>
                    );
                })
            }
        ...
        <div className={['container-fluid'].join(' ')}>
            <div className={['row'].join(' ')}>
                <div className={['col-12', 'd-flex', 'flex-row', 'align-items-center', 'rtl', 'justify-content-right'].join(' ')}>
                    ...
                    <button className={['px-2', 'py-1', 'mr-2', getSectionDisplayClass(1)].join(' ')} onClick={() => {setSelectedSectionId(1)}}>افزودن محصولات</button>
